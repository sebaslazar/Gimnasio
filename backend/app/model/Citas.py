...
class Cita(SQLModel, TimeMixin, table=True):
    ...
    ID_cliente: Optional[str] = Field(default=None, foreign_key="cliente.ID_cliente", nullable=False)
    cliente: Optional["Cliente"] = Relationship(back_populates="citas")

    ID_actividad: Optional[str] = Field(default=None, foreign_key="actividad.ID_actividad", nullable=False)
